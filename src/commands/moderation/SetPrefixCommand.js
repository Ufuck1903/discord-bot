...
module.exports = class SetPrefixCommand extends BaseCommand {
    ...
    async run(client, message, args) {
        ...
        if (!message.member.permissions.has("MANAGE_GUILD")) { //check user permission
            return message.channel.send("You don't have the permissions to use this command! (MANAGE_GUILD)");
        ...
        if (check === false) return message.channel.send("Can only contain symbol numbers and letters.");
        const filter = {_id: message.guild.id};
        const update = {prefix: args[0]};
        let doc = await guildModel.findOneAndUpdate(filter, update, {useFindAndModify: false});// update user arg on database
